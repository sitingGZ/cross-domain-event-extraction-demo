...
def create_trigger_identification_annotation(data_point, ds_key="geneva", identification_task_prompt = "Identify trigger words.", classification_e2e_task_prompt = "Classify trigger types end-to-end.", classification_task_prompt = "Classify trigger types.",  use_human_readable=False, augument_word_generation=False):
  """
 ...
 The labels are constructed as
  ...
  for idx, (s, e) in enumerate(spans):
    ...
    if use_human_readable:
      class_labels_e2e.append(f"trigger> {span_text}>={human_name}")
      class_labels.append(f"{span_text}={human_name}")
    else:
      class_labels_e2e.append(f"trigger> {span_text}>={primary if primary else 'Unknown'}")
      class_labels.append(f"{span_text}={primary if primary else 'Unknown'}")
  #class_labels.append(f"<extra_id_{len(spans)}>")
  classification_e2e_output = " | ".join(class_labels_e2e)
  classification_pipeline_output = " | ".join(class_labels)
  ...
  results['identification'] = { 'input': f"Domain: {ds_key}. {identification_task_prompt} {plain_text}", 'output': tagged_sentence}
  results['classification_pipeline'] = { 'input': f"Domain: {ds_key}. {classification_task_prompt} {tagged_sentence}", 'output': classification_pipeline_output}
  results['classification_e2e'] = { 'input': f"Domain: {ds_key}. {classification_e2e_task_prompt} {plain_text}", 'output': classification_e2e_output}
...
def create_event_extraction_e2e_annotation(data_point, ds_key="geneva", e2e_task_prompt = "Event trigger and argument extraction.", use_human_readable=False):
  """
  ...
  Pattern explanation:
      ...
      - This makes parsing unambiguous: split by '|' for events, 
  """
  tokens = data_point.get("tokens")
  event_mentions = data_point.get("event_mentions", [])
  ...
  plain_text = _clean_space_punct(" ".join(tokens))
  e2e_input = f"Domain: {ds_key}. {e2e_task_prompt} {plain_text}"
...
def create_argument_extraction_annotation(doc_id, data_point, ds_key="geneva", argument_identification_task_prompt = "Identify potential arguments.", argument_extraction_task_prompt = "Extract arguments for a specific event.", use_human_readable=False):
  """
  ...
  plain_text = _clean_space_punct(" ".join(tokens))
  identification_input = f"Domain: {ds_key}. {argument_identification_task_prompt} {plain_text}"
  ...
  for ev in event_mentions:
    ...
    for arg in args:
      ...
      span_text = _get_span_text_from_arg(arg)
      if span_text:
        all_arg_pairs.append(f"argument>{span_text}>={role}({event_type if event_type else 'Unknown'})")

  identification_output = None if not all_arg_pairs else " | ".join(all_arg_pairs)
  argument_identification = { 'doc_id': doc_id, 'input': identification_input, 'output': identification_output} if identification_output else None
  ...
  for ev_idx, ev in enumerate(event_mentions):
    ...
    if use_human_readable:
      ...
      extraction_input = f"Domain: {ds_key}. {argument_extraction_task_prompt} argument> {roles_str} for trigger> {span_text}>={human_name}: {plain_text}"
    else:
      extraction_input = f"Domain: {ds_key}. {argument_extraction_task_prompt} argument> {roles_str} for trigger> {span_text}>={event_type}: {plain_text}"
  ...
  for split_name, split_file in splits.items():
    ...
    for i, item in enumerate(data):
      ...
      if trig_ann:
        ...
        clas_e2de = trig_ann.get('classification_e2e')
        
        ident_aug = trig_ann.get('identification_augument')
        if ident_aug:
          tasks['trigger_identification_augument'][split_name].append({ 'doc_id': f"doc_{i}", 'input': ident_aug['input'], 'output': ident_aug['output']})
        if ident:
          tasks['trigger_identification'][split_name].append({'doc_id': f"doc_{i}", 'input': ident['input'], 'output': ident['output']})
        if clas_pipeline:
          tasks['trigger_classification_pipeline'][split_name].append({'doc_id': f"doc_{i}", 'input': clas_pipeline['input'], 'output': clas_pipeline['output']})
        if clas_e2de:
          tasks['trigger_classification_e2e'][split_name].append({'doc_id': f"doc_{i}", 'input': clas_e2de['input'], 'output': clas_e2de['output']})
      ...
      try:
        argument_identification, argument_extraction_list = create_argument_extraction_annotation(f"doc_{i}", item, ds_key=ds_key)
      except Exception:
  ...
  for split_name, split_file in splits.items():
    ...
    for i, item in enumerate(data):
      try:
        ...
        e2e_ann = None
      if e2e_ann:
        tasks['event_extraction_e2e'][split_name].append({'doc_id': f"doc_{i}", 'input': e2e_ann['input'], 'output': e2e_ann['output']})
  return tasks
