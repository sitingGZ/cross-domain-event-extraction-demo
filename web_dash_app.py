...
# (ported from web_insights_dashboard.py)
# --------------------------------------------------
...
def _api_request(path: str, payload=None, method: str = "POST"):
...
def _safe_get(record, keys, default: str = "") -> str:
    for key in keys:
...
def _decode_upload(contents: str):
...
def _has_inference_results(record) -> bool:
...
def _infer_record(record, default_dataset: str, default_model_type: str, max_length: int) -> dict:
...
def _select_modes(result, selected_modes: Sequence[str]) -> list:
...
def _extract_events(result, mode: str) -> list:
...
def _trigger_sequences(result, mode: str) -> list:
...
def _count_events(events, event_type_filter: Sequence[str], role_filter: Sequence[str]):
    trigger_counter = Counter()
    event_type_counter = Counter()
    role_counter = Counter()
    trigger_word_counter = Counter()
    argument_text_counter = Counter()
...
def _build_kpi(title: str, value, subtitle: str) -> dbc.Card:
    return dbc.Card(
        dbc.CardBody([
...
def _event_cards(result, selected_modes: Sequence[str], event_type_filter: Sequence[str], role_filter: Sequence[str]):
    cards = []
...
def _render_schema_overview(results):
...
def _render_documents_table(results, selected_modes, event_type_filter, role_filter):
    rows = []
    for result in results:
...
def _merge_analysis_runs(existing, new_runs):
    merged = {}
...
def cache_uploaded_records(contents, filename):
    if not contents:
        raise PreventUpdate
...
def load_sample_records_cb(n_clicks):
    if not n_clicks:
        raise PreventUpdate
...
# Expose insights page via the main app router
def create_insights_layout_page():
    return _build_insights_layout()


@app.callback(Output("page-content", "children"), Input("url", "pathname"))
def render_page_with_insights(pathname):
    if pathname in ("/", "/intro", None):
        return create_intro_layout()
    if pathname in ("/playground", "/pg"):
        return create_playground_layout()
    if pathname in ("/app", "/extract", "/extraction"):
        return create_extraction_layout()
    if pathname in ("/dashboard", "/insights"):
        return create_insights_layout_page()
    return dbc.Container([
        html.H2("Page not found", className="mt-4"),
        html.P("Return to the introduction page."),
        dbc.Button("Go to Introduction", href="/", color="secondary")
